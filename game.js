...
const BTYPES={HOSPITAL:{color:0xffffff,accent:0xef4444,nameAr:'مستشفى الأمل',desc:'مبنى أبيض طويل بخطوط حمراء',colorName:'أبيض'},SCHOOL:{color:0xfacc15,accent:0x1e40af,nameAr:'مدرسة النور',desc:'مبنى أصفر بخطوط زرقاء',colorName:'أصفر'},FACTORY:{color:0x475569,accent:0x1e293b,nameAr:'مصنع الغد',desc:'مبنى رمادي بمدخنة',colorName:'رمادي'},MALL:{color:0xec4899,accent:0xbe185d,nameAr:'مول الخليج',desc:'مبنى وردي لامع',colorName:'وردي'},MOSQUE:{color:0x10b981,accent:0x065f46,nameAr:'مسجد القرية',desc:'مبنى أخضر بقبة ذهبية',colorName:'أخضر'},TOWER:{color:0x334155,accent:0x60a5fa,nameAr:'برج سكني',desc:'برج زجاجي شاهق',colorName:'رمادي'},HOUSE:{color:0xf1f5f9,accent:0x991b1b,nameAr:'منزل سكني',desc:'منزل أبيض بسطح أحمر',colorName:'أبيض'}};
...
// الأسباب الجذرية للبغ:
// 1. guessing: flag يمنع إرسال guess مرتين
// 2. lastHandledAttempt: يمنع معالجة result أكثر من مرة
// 3. lastHandledGuess: يمنع معالجة guess أكثر من مرة
let G={name:'',role:'host',myScore:0,opScore:0,oppName:'؟',roomCode:'',hidden:null,selected:null,attempts:3,phase:'lobby',timerV:60,timerI:null,citySeed:0,guessing:false,lastHandledAttempt:-1,lastHandledGuess:-1};
...
function handleMsg(type,data){
  ...
  else if(type==='start_search'){G.hidden=data.cell;G.phase='seeking';G.lastHandledAttempt=-1;updateUI();setStatus('المضيف أخفى الكنز — دورك تبحث! اضغط على مبنى');document.getElementById('seeker-ctrl').style.display='block';document.getElementById('hider-ctrl').style.display='none';document.getElementById('att-row').style.display='flex';hideBuildingInfo();updateDots();updateBadge();startTimer();}
  else if(type==='guess'){
    // المضيف يتجاهل التخمينات المكررة لنفس المحاولة
    if(data.attempt===G.lastHandledGuess){return;}
    G.lastHandledGuess=data.attempt;
    handleGuess(data.cell,data.attempt);
  }
  else if(type==='result'){
    // الزائر يتجاهل النتائج المكررة لنفس المحاولة
...
async function confirmHide(){if(!G.selected)return;G.hidden=G.selected;G.phase='ready_to_search';updateUI();const b=selectableBuildings.find(x=>x.id===G.selected);if(b){b.mesh.material.color.setHex(0xffd700);b.grp.userData.marked=true;addGemMarker(b);showBuildingInfo(b,true);}setStatus('تم تحديد مخبأ الكنز ✔️ — اضغط ابدأ البحث');document.getElementById('btn-hide').disabled=true;document.getElementById('btn-start-search').style.display='block';}
async function startSearch(){if(!G.hidden)return;await sendEvent({t:'start_search',cell:G.hidden});G.phase='host_waiting';G.lastHandledGuess=-1;updateUI();setStatus('الزائر بيبحث... انتظر');document.getElementById('btn-start-search').style.display='none';document.getElementById('hider-ctrl').style.display='none';hideBuildingInfo();}

// FIX: منع إرسال guess أكثر من مرة باستخدام guessing flag
async function confirmGuess(){
  if(!G.selected||G.attempts<=0||G.guessing)return;
  G.guessing=true; // امنع أي ضغطة تانية
  G.attempts--;
  ...
  G.selected=null;
  hideBuildingInfo();
  // G.guessing يبقى true حتى يجي الـ result
}

// FIX: المضيف يعالج guess مرة واحدة فقط
...
// FIX: applyResult بتششغل مرة واحدة فقط (محمية بالـ lastHandledAttempt)
function applyResult(found,guessCell,pts,hiddenCell,attempt){
  G.guessing=false; // سمح للزائر يخمّن تاني
  if(!found){
...
function showEnd(isSeeker,found,pts){stopTimer();const isGuest=G.role==='guest';document.getElementById('ov-icon').textContent=found?'🎉':'😅';if(found){document.getElementById('ov-title').textContent=isGuest?'وجدت الكنز! 🎉':'الزائر وجد الكنز';document.getElementById('ov-msg').textContent=isGuest?'ربحت '+pts+' نقطة!':'الزائر اكتشف المخبأ';}else{document.getElementById('ov-title').textContent=isGuest?'لم تجد الكنز 😅':'ربحت الجولة! 🏆';document.getElementById('ov-msg').textContent=isGuest?'فشلت في إيجاد الكنز':'الزائر فشل — ربحت 20 نقطة!';}document.getElementById('end-overlay').style.display='flex';hideBuildingInfo();saveScore();}
...
function applyNextRound(newRole,newSeed){G.role=newRole;G.hidden=null;G.selected=null;G.attempts=3;G.guessing=false;G.lastHandledAttempt=-1;G.lastHandledGuess=-1;G.phase=newRole==='host'?'hiding':'waiting_host';document.getElementById('end-overlay').style.display='none';document.getElementById('btn-start-search').style.display='none';document.getElementById('btn-hide').disabled=true;document.getElementById('btn-guess').disabled=true;document.getElementById('btn-guess').textContent='🔍 تأكيد التخمين';if(newSeed)G.citySeed=newSeed;rng=makeRng(G.citySeed);clearMarkers();resetBuildingColors();generateCity();updateUI();hideBuildingInfo();if(G.role==='host')setStatus('دورك تخفي الكنز — اختر مبنى');else setStatus('في انتظار المضيف...');}
function showBuildingInfo(b,isHidden,isWrong){const panel=document.getElementById('building-info');const colorHex='#'+(b.grp.userData.origColor||0x888888).toString(16).padStart(6,'0');let icon='🏙️';if(b.nameAr.includes('مستشف'))icon='🏥';else if(b.nameAr.includes('مدرس'))icon='🏫';else if(b.nameAr.includes('مصنع'))icon='🏷️';else if(b.nameAr.includes('مول'))icon='🛍️';else if(b.nameAr.includes('مسجد'))icon='🕌';else if(b.nameAr.includes('برج'))icon='🏙️';else if(b.nameAr.includes('منزل'))icon='🏠';let statusBadge='';if(isHidden)statusBadge='<span class="info-badge gem">💎 مخبأ الكنز</span>';if(isWrong)statusBadge='<span class="info-badge wrong">❌ ليس هنا</span>';panel.innerHTML='<div class="info-row"><span class="info-icon">'+icon+'</span><div class="info-content"><div class="info-name">'+b.nameAr+' '+statusBadge+'</div><div class="info-desc">'+(b.desc||'مبنى في المدينة')+'</div><div class="info-color"><span class="color-dot" style="background:'+colorHex+'"></span>'+(b.colorName||'')+'</div></div></div>';panel.style.display='block';}
...
function updateUI(){const rb=document.getElementById('role-banner');document.getElementById('my-lbl').textContent=G.name||'أنا';document.getElementById('op-lbl').textContent=G.oppName;document.getElementById('hider-ctrl').style.display='none';document.getElementById('seeker-ctrl').style.display='none';document.getElementById('att-row').style.display='none';document.getElementById('btn-start-search').style.display='none';if(G.role==='host'){rb.className='role-banner role-hide';rb.textContent='🏠 أنت المضيف — دورك الإخفاء';if(G.phase==='hiding')document.getElementById('hider-ctrl').style.display='block';}else{rb.className='role-banner role-seek';rb.textContent='🚶 أنت الزائر — دورك البحث';if(G.phase==='seeking'){document.getElementById('seeker-ctrl').style.display='block';document.getElementById('att-row').style.display='flex';updateDots();updateBadge();}}updateScores();}
